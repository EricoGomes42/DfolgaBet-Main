...
export function applyDynamicContentToResult(obj: any): any {
  ...
  if (obj !== null && typeof obj === 'object') {
    ...
    for (const [key, value] of Object.entries(obj)) {
      if (['slug', 'url', 'href', 'current', 'link', '_id', '_ref', '_key', 'seoCustomCode'].includes(key)) {
        newObj[key] = value;
      } else {
...
console.log(applyDynamicContentToResult({
  title: "Teste {ano}",
  slug: { current: "teste-{ano}" },
  categories: ["Cat 1 {ano}"],
  author: { name: "Author {ano}" }
}));
