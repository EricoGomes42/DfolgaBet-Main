...
async function run() {
    try {
        ...
        const faqItemsForJsonLd = filteredBody.filter(b => b._type === 'faqItem').map(b => ({
            "@type": "Question",
            "name": b.question,
            "acceptedAnswer": {
              "@type": "Answer",
              "text": typeof b.answerBlocks === 'string'
                ? b.answerBlocks
                : b.answerBlocks ? b.answerBlocks.map((blk) => blk.children?.map((c) => c.text).join('')).join(' ') : ''
            }
        }));
          
        console.log("FAQ ITEMS FOR JSON-LD:");
        console.log(JSON.stringify(faqItemsForJsonLd, null, 2));
    } catch(e) {
        console.error(e);
    }
}
