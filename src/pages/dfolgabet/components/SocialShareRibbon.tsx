...
export default function SocialShareRibbon() {
  ...
  React.useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      setCurrentTitle(encodeURIComponent(document.title));
    }
  }, []);
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 mb-8 mt-4">
      ...
      <a href={currentUrl ? `https://wa.me/?text=${currentTitle}%20${currentUrl}` : "#"} target="_blank" rel="noopener noreferrer" className="bg-[#25D366] text-white w-9 h-9 rounded-md flex items-center justify-center hover:opacity-90 transition-opacity" aria-label="WhatsApp">
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 21l1.65-3.8a9 9 0 1 1 3.4 2.9L3 21"/><path d="M9 10a.5.5 0 0 0 1 0V9a.5.5 0 0 0-1 0v1a5 5 0 0 0 5 5h1a.5.5 0 0 0 0-1h-1a.5.5 0 0 0 0 1"/></svg>
      </a>
      <a href={currentUrl ? `https://www.facebook.com/sharer/sharer.php?u=${currentUrl}` : "#"} target="_blank" rel="noopener noreferrer" className="bg-[#1877F2] text-white w-9 h-9 rounded-md flex items-center justify-center hover:opacity-90 transition-opacity" aria-label="Facebook">
      ...
      <a href={currentUrl ? `https://twitter.com/intent/tweet?url=${currentUrl}&text=${currentTitle}` : "#"} target="_blank" rel="noopener noreferrer" className="bg-black text-white w-9 h-9 rounded-md flex items-center justify-center hover:opacity-90 transition-opacity" aria-label="X">
      ...
      <a href="#" onClick={handleInstagramClick} className="bg-gradient-to-tr from-[#f09433] via-[#e6683c] via-[#dc2743] via-[#cc2366] to-[#bc1888] text-white w-9 h-9 rounded-md flex items-center justify-center hover:opacity-90 transition-opacity" aria-label="Instagram">
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line></svg>
      </a>
      <a href={currentUrl ? `https://t.me/share/url?url=${currentUrl}&text=${currentTitle}` : "#"} target="_blank" rel="noopener noreferrer" className="bg-[#0088cc] text-white w-9 h-9 rounded-md flex items-center justify-center pl-0.5 pt-0.5 hover:opacity-90 transition-opacity" aria-label="Telegram">
      ...
      <a href={currentUrl ? `https://www.linkedin.com/sharing/share-offsite/?url=${currentUrl}` : "#"} target="_blank" rel="noopener noreferrer" className="bg-[#0A66C2] text-white w-9 h-9 rounded-md flex items-center justify-center hover:opacity-90 transition-opacity" aria-label="LinkedIn">
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg>
      </a>
      <a href={currentUrl ? `https://pinterest.com/pin/create/button/?url=${currentUrl}` : "#"} target="_blank" rel="noopener noreferrer" className="bg-[#E60023] text-white w-9 h-9 rounded-md flex items-center justify-center hover:opacity-90 transition-opacity" aria-label="Pinterest">
      ...
      <a href="https://utimeoff.com/" target="_blank" rel="noopener noreferrer" className="group relative block w-9 h-9 rounded-md overflow-hidden bg-gradient-to-br from-[#D99A5A] to-[#619B9D]" aria-label="uTimeOff">
         <div className="absolute inset-0 flex items-center justify-center transition-opacity group-hover:opacity-0 delay-75">
           <img src="/assets/logos/partners/utimeoff-logo.webp" alt="uTimeOff" className="w-[18px] h-[18px] object-contain" style={{ filter: 'brightness(0) invert(1)' }} />
         </div>
         <div className="absolute inset-0 bg-[#50C0CC] opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity duration-200">
            <div 
              ...
              style={{ maskImage: `url(/assets/logos/partners/utimeoff-logo.webp)`, maskSize: 'contain', maskRepeat: 'no-repeat', maskPosition: 'center', WebkitMaskImage: `url(/assets/logos/partners/utimeoff-logo.webp)`, WebkitMaskSize: 'contain', WebkitMaskRepeat: 'no-repeat', WebkitMaskPosition: 'center' }}
            />
         </div>
      </a>
    </div>
